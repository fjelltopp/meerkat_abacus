...
global engine
global session
session, engine = data_management.set_up_database(leave_if_data=False,
                                                  drop_db=True)
...
if config.fake_data:
    tasks.add_fake_data.delay(N=4, countdown_time=config.fake_data_interval, dates_is_now=True)
