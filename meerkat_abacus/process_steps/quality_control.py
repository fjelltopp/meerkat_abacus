...
def quality_control(form, row, param_config, config_object):
    """ 

    Does quality control to change any needed data and to check that data should be added

    Args:
        form: form_name
        row: data_row
        config object with the following data:
    ...
    """
    config = config_object[form]
    exclusion_list = set(config["exclusion_list"])
    ...
    if row[config["uuid_field"]] in exclusion_list:
        return None

    if config["row_function"]:
        insert_row = config["row_function"](row)
    else:
        insert_row = row
...
def prepare_arguments(form, session, param_config):
